...
function highlightMissingAsset(globalX, globalY) {
	...
	$.ajax({url: url,  crossDomain: true, async: false}).done(function(result) {
...
function getOpenCaseMarker(){
       if (KDF.getVal('txt_eventcode')) { 
                ...
                KDF.customdata('open_case_marker', 'create', true, true, {'eventcode': KDF.getVal('txt_eventcode'),'customerid': sx_customer_id});
                KDF.unlock();
             }   
}
